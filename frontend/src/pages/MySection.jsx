...
import { Users, BookOpen, QrCode, Trash2, Plus } from 'lucide-react';
...
const StudentList = ({ entry }) => {
  ...
  if (loading) return <div className="p-10 text-center text-slate-400 text-sm">Loading students...</div>;
  if (students.length === 0)
    return <div className="p-10 text-center text-slate-400 text-sm">No students in this section yet.</div>;
    ...
    <div className="divide-y divide-white/5">
      {students.map((s, i) => (
        <div key={s.id} className="flex items-center gap-3 px-5 py-3 hover:bg-white/5 transition-colors">
          ...
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${s.active ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
...
const MySection = () => {
  ...
  const handleRemove = async (id, idx) => {
    try {
      await axiosInstance.delete(`/faculty/my-sections/${id}`);
  ...
  if (loading) return <div className="p-10 text-center text-slate-400">Loading sections...</div>;
      ...
      <div className="flex gap-2 flex-wrap">
        {sections.map((entry, i) => (
          ...
          >
            ...
            <span
              onClick={(e) => { e.stopPropagation(); handleRemove(entry.id, i); }}
      ...
      {active && (
        ...
        >
          ...
          <div className="px-5 py-4 flex items-center gap-3 flex-wrap border-b border-white/10">
            <span className="bg-blue-500/20 text-blue-300 text-xs font-bold px-3 py-1 rounded-full">{active.department}</span>
            <span className="bg-purple-500/20 text-purple-300 text-xs font-bold px-3 py-1 rounded-full">Year {active.year}</span>
            <span className="bg-cyan-500/20 text-cyan-300 text-xs font-bold px-3 py-1 rounded-full">Section {active.section}</span>
            {active.subjectName && (
              <span className="bg-green-500/20 text-green-300 text-xs font-medium px-3 py-1 rounded-full flex items-center gap-1">
                <BookOpen className="w-3 h-3" />{active.subjectName}
              </span>
            )}
